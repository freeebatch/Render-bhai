            ...
            if courses:
                if total > 50:
                    ...
                    for cnt, course in enumerate(courses):
                        ...
                        price = course["price"]
                        text += f'{cnt + 1}. ```\n{name} 💵₹{price}```\n'
                    editable = await m.reply_text("**Send one or more index numbers from the course details file (e.g., `1 3 5`) to download.**")
            else:
...
if not selected_indices:
    await editable.edit(" Wrong index number(s). Please try again.")
    return
...
for selected_course_index in selected_indices:
    ...
    await m.reply_text(f" Extracting course: **{selected_batch_name}**")
    ...
    try:
        if folder_wise_course == 0:
            ...
            all_outputs.extend(outputs_0 + outputs_1)

        if not all_outputs:
            await m.reply_text(f" No content found in: **{selected_batch_name}**")
            continue
        ...
        await m.reply_text(f" Failed to extract: **{selected_batch_name}**\nError: `{str(e)}`")
