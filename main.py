...
def find_pw_old_batch(batch_search, message):
    import requests
    import asyncio
    import os
    import zipfile
...
ðŸ“¥ **Enter range like `0-3` to extract multiple batches:**"
    ...
    user_input = loop.run_until_complete(ask_range())

    try:
        start_idx, end_idx = map(int, user_input.split("-"))
        selected_batches = matching_batches[start_idx:end_idx+1]
    except:
        selected_batches = [matching_batches[0]]

    # Run extraction and create one zip
    zip_filename = "combined_batches.zip"
    with zipfile.ZipFile(zip_filename, 'w') as zipf:
        for batch in selected_batches:
            batch_name_clean = batch['batch_name'].replace(" ", "_")
            txt_filename = f"{batch_name_clean}.txt"
            json_filename = f"{batch_name_clean}.json"

            # Create dummy txt and json files as placeholders
            with open(txt_filename, "w", encoding="utf-8") as f:
                f.write(f"Subjects for {batch['batch_name']}
")
                for subj in batch.get("subjects", []):
                    f.write(f"- {subj['subject_name']}: {subj['link']}
")
...
"
    ...
    async def ask_range():
        await message.reply(batch_list_text + "
**Enter range like `0-2` to extract multiple batches:**")
        response_msg = await message.chat.listen()
        return response_msg.text.strip()
    ...
    try:
        start_idx, end_idx = map(int, user_input.split("-"))
        return matching_batches[start_idx:end_idx+1]
    except:
        return [matching_batches[0]]  # fallback to the first if input invalid


    try:
        response = requests.get(f"https://freeebatch.github.io/Data/data1.json")
        response.raise_for_status()
        data = response.json()
    ...
    try:
        ...
        async with aiohttp.ClientSession() as session:
            for batch in data:
                ...
                batch_id = batch['_id']
                subject_list = batch.get("subjects", [])

                json_data = defaultdict(dict)
                all_subject_urls = defaultdict(list)
                ...
                for subject in subject_list:
                    await process_pwwp_subject(
                        ...
                        selected_batch_name=batch_name,
                        zipf=None,
                        json_data=json_data,
                        all_subject_urls=all_subject_urls,
                        headers=headers
                    )
